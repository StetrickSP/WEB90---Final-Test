...
const CreationItem = ({ item } : CreationProps) => {

  const [ expanded, setExpanded ] = useState(false);

  return (
    <div className='p-4 max-w-5xl text-sm bg-white border border-gray-200 rounded-lg transition-all duration-300'>
      <div  onClick={()=> setExpanded(!expanded)} className='flex justify-between items-center gap-4 cursor-pointer hover:bg-gray-50'>
        <div>
            <h2>{item.prompt}</h2>
